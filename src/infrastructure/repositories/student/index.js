...
module.exports = (dbContext) => {
  const model = dbContext.models.student;
  ...
  const findByCpf = async (cpf) => {
    ...
    if (!queryResult) {
      throw NotFoundException("Nenhum aluno(a) encontrado!");
    }
  ...
  const update = async (data) => {
    ...
    if (!entityToUpdated) {
      throw NotFoundException("Aluno(a) não encontrado!");
    }
  ...
  const prepareQuerySearch = (filters, hasTotal) => {
    let query = ` select s.id, s.cpf, s.student_name as studentname, s.photo, s.birthdate, s.entry_time as entryTime, s.deperture_time as depertureTime,s.deleted_at as deletedAt
                  from student s
                `;
    if (hasTotal) {
      query = `select count(distinct id) as total from student`;
    }

    if (filters) {
      let queryFilters = [];
      if (filters.studentName) {
        queryFilters.push(` lower(student_name) like '%${filters.studentName.toLowerCase()}%'`);
      }
      ...
      if (!hasTotal) {
        query = query + ` order by student_name ASC limit ${limit} offset ${offset};`;
      }
    }
