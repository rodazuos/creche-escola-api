...
module.exports = (dbContext) => {
  const model = dbContext.models.routine_type_detail;
  ...
  const prepareQuerySearch = (filters, hasTotal) => {
    let query = ` select rtd.id, rtd.id_routine_type as idRoutineType, rt.title as titleRoutineType,
                  rtd.title, rtd.deleted_at as deletedAt
                  from routine_type_detail rtd
                    inner join routine_type rt on rtd.id_routine_type = rt.id
                `;
    if (hasTotal) {
