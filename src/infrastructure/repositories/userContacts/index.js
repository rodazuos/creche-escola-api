...
module.exports = (dbContext) => {
  const model = dbContext.models.user_contacts;
  ...
  const getById = async (id, { includeDeleted = false } = {}) => {
    ...
    if (!queryResult) {
      throw NotFoundException("Nenhum contato encontrado!");
    }
  ...
  const prepareQuerySearch = (filters, hasTotal) => {
    let query = ` select id, id_user, email, house_phone_number as housePhoneNumber, personal_phone_number as personalPhoneNumber,
                commercial_phone_number as commercialPhoneNumber, contact_description as contactDescription, deleted_at as deletedAt
                  from user_contacts
                `;
    if (hasTotal) {
      query = `select count(distinct id) as total from user_contacts`;
    }
