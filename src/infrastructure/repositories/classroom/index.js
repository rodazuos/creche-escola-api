...
module.exports = (dbContext) => {
  const model = dbContext.models.classroom;
  ...
  const prepareQuerySearch = (filters, hasTotal) => {
    let query = ` select id, title, description, teacher, deleted_at as deletedAt
                  from classroom
                `;
    if (hasTotal) {
      query = `select count(distinct id) as total from classroom`;
    }
    ...
    if (filters) {
      ...
      if (filters.teacher) {
        queryFilters.push(` lower(teacher) like '%${filters.teacher.toLowerCase()}%'`);
      }
