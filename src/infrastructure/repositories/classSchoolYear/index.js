...
module.exports = (dbContext) => {
  const model = dbContext.models.class_school_year;
  ...
  const getById = async (id, { includeDeleted = false } = {}) => {
    ...
    if (!queryResult) {
      throw NotFoundException("Nenhum ano letivo encontrada!");
    }
  ...
  const findByYearAndClass = async (schoolYear, idStudentClass) => {
    const whereConditions = { [Op.and]: [{ schoolYear }, { idStudentClass }] };
    const queryResult = await model.findOne({ where: whereConditions });
  ...
  const update = async (data) => {
    ...
    if (!entityToUpdated) {
      throw NotFoundException("Ano letivo não encontrado!");
    }
  ...
  const getList = async (filters) => {
    ...
    if (!queryResult) {
      throw NotFoundException("Nenhum ano letivo encontrado!");
    }
  ...
  const prepareQuerySearch = (filters, hasTotal) => {
    let query = ` select csy.id, csy.school_year as schoolYear, csy.id_students_class as idStudentsClass, sc.class_name as className,
                  csy.start_time as startTime, csy.end_time as endTime, sc.deleted_at as deletedAt
                  from class_school_year csy
                    inner join students_class sc on csy.id_students_class = sc.id
                `;
    if (hasTotal) {
      query = `select count(distinct id) as total from class_school_year`;
    }

    if (filters) {
      let queryFilters = [];
      if (filters.schoolYear) {
        queryFilters.push(` school_year = ${filters.schoolYear}`);
      }
      ...
      if (!hasTotal) {
        query = query + ` order by school_year, class_name ASC limit ${limit} offset ${offset};`;
      }
    }
