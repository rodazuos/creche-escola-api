...
module.exports = (dbContext) => {
  const model = dbContext.models.student_daily_routine;
  ...
  const prepareQuerySearch = (filters, hasTotal) => {
    let query = ` select sdr.id, 
                  sdr.id_student_daily as studentDaily, sd.date_daily as dateDaily,
                  sdr.id_routine_type as idRoutineType, rt.title,
                  sdr.id_routine_type_detail as idRoutineTypeDetail, rtd.title as titleRoutineDetail,
                  sdr.start_hour_routine as startHourRoutine, sdr.end_hour_routine as endHourRoutine, sdr.description
                  sdr.deleted_at as deletedAt
                  from student_daily_routine sdr
                    inner join student_daily sd on sdr.id_student_daily = sd.id
                    inner join routine_type rt on sdr.id_routine_type = rt.id
                    inner join routine_type_detail rtd on sdr.id_routine_type_detail = rtd.id
                `;
    if (hasTotal) {
      query = `select count(distinct sdr.id) as total from student_daily_menu sdr
                  inner join student_daily sd on sdr.id_student_daily = sd.id
                  inner join routine_type rt on sdr.id_routine_type = rt.id
                  inner join routine_type_detail rtd on sdr.id_routine_type_detail = rtd.id
      `;
